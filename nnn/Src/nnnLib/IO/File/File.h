...
NNN_API HRESULT	get_file_info(const WCHAR *filename, __out struct _stat64i32 *fi);
#else
...
// 从绝对路径中提取文件名
NNN_API HRESULT	remove_file_path(const WCHAR *full_path, __out WCHAR filename[MAX_PATH]);
NNN_API HRESULT	remove_file_path(const char *full_path, __out char filename[MAX_PATH]);

// 去除文件名的后缀
NNN_API HRESULT	remove_file_extension(const WCHAR *filename, __out WCHAR output_filename[MAX_PATH]);
NNN_API HRESULT	remove_file_extension(const char *filename, __out char output_filename[MAX_PATH]);
